...
%         cluster_variance(i,:) = var(temp);
    ...
    [errorVal2, ermsValidation(1,M2)] = calculateError(phiValid, validationT2, w2, size(validationX2, 1), 0);
    
    % testing set
    phiTest = calculatePhi(testingX2, M2, Sigma2, mu2);
    [errorTest2, ermsTest(1,M2)] = calculateError(phiTest, testingT2, w2, size(testingX2, 1), 0);
